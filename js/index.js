...
let techNum = 0;
let langNumColor;
let langNumColorLenght = 0;
const fetchColor = async () => {
  const color = JSON.parse(colorJson);
  return color;
};
fetchColor().then((e) => {
  langNumColorLenght = Object.keys(e).length;
});

const content = () => {
  if (num == skills.length) num = 0;
  if (techNum == languages.length) techNum = 0;
  icon.textContent = skills[num][0];
  skill.textContent = skills[num][1];
  lang.textContent = languages[techNum].toLocaleUpperCase();
  num++;
  techNum++;
  fetchColor().then((e) => {
    lang.style.color =
      Object.keys(e)[Math.floor(langNumColorLenght * Math.random())];
    skill.style.color =
      Object.keys(e)[Math.floor(langNumColorLenght * Math.random())];
  });
  return;
